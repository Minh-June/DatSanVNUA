...
<?php $__env->startSection('title', 'Giỏ hàng sản phẩm'); ?>
        ...
        <div class="cart-detail-wrapper">
            ...
            <div class="cart-detail-left">
                <table id="ListCustomers" style="margin-bottom:60px;">
                    ...
                    <tbody>
                        <?php $__currentLoopData = $groupedBuys; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $storeId => $storeItems): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <?php
                            ...
                            <tr class="name-shop-cart-pay" onclick="window.location='<?php echo e(route('chi-tiet-cua-hang', $storeId)); ?>'" style="cursor:pointer;">
                                <td colspan="6" class="left-align">
                                    <img src="<?php echo e($store && $store->user && $store->user->image ? asset('storage/'.$store->user->image) : asset('images/default-avatar.png')); ?>" 
                            ...
                            <?php $__currentLoopData = $productsGrouped; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $items): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <?php
                                ...
                                <tr id="row_<?php echo e($sessionKey); ?>" data-group-key="<?php echo e($storeId); ?>_<?php echo e($firstItem['product_id']); ?>">
                                    <td style="cursor:pointer;" onclick="window.location='<?php echo e(route('chi-tiet-san-pham',$firstItem['product_id'])); ?>'">
                                        <img src="<?php echo e(asset('storage/' . ($firstItem['image'] ?? 'image/football.jpg'))); ?>" width="80">
                                    </td>
                                    <td class="left-align product-name-cart" style="cursor:pointer;" onclick="window.location='<?php echo e(route('chi-tiet-san-pham',$firstItem['product_id'])); ?>'">
                                    ...
                                    <td><?php echo e(number_format($firstItem['price'])); ?>đ</td>
                                    <td>
                                        <div class="product-quantity" style="margin:0 0 0 20px;">
                                            <button type="button" class="quantity-btn" onclick="changeQuantity('<?php echo e($sessionKey); ?>', <?php echo e($firstItem['price']); ?>, -1)">-</button>
                                            <input class="quantity-cart" id="quantityInput_<?php echo e($sessionKey); ?>" value="<?php echo e($totalQuantity); ?>" readonly>
                                            <button type="button" class="quantity-btn" onclick="changeQuantity('<?php echo e($sessionKey); ?>', <?php echo e($firstItem['price']); ?>, 1)">+</button>
                                        </div>
                                    </td>
                                    <td id="totalPrice_<?php echo e($sessionKey); ?>"><?php echo e(number_format($totalPrice)); ?>đ</td>
                                    <td style="text-align:center;">
                                        <button type="button" class="delete-btn" onclick="deleteItemGroup('<?php echo e($storeId); ?>_<?php echo e($firstItem['product_id']); ?>')">Xóa</button>
                                    </td>
                                </tr>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            ...
                            <tr>
                                ...
                                <td colspan="2" style="font-size:17px; font-weight:bold;" id="shopTotal_<?php echo e($storeId); ?>">0đ</td>
                            </tr>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </tbody>
                </table>
            ...
            <div class="cart-detail-right">
                <div class="cart-summary">
                    ...
                    <p><strong class="cart-dash">Tổng</strong> <span id="cartTotal"><?php echo e(number_format($totalPriceAll)); ?>đ</span></p>
                </div>
            </div>
        </div>

        <div class="footer-link" style="margin-bottom:40px;">
            <a href="<?php echo e(route('thanh-toan-gio-hang')); ?>">
...
document.addEventListener('DOMContentLoaded', function(){
    if(<?php echo e(count($buys)); ?>===0){
        alert('Vui lòng quay lại trang mua sắm mua sản phẩm !');
        window.location.href = "<?php echo e(route('danh-sach-cua-hang')); ?>";
    }
    updateCartTotal();
});
...
function changeQuantity(key, price, delta){
    ...
    fetch("<?php echo e(route('cap-nhat-so-luong')); ?>",{
        method:'POST',
...
function deleteItemGroup(groupKey){
    ...
    indexes.forEach(index=>{
        fetch("<?php echo e(route('xoa-san-pham-tam-thoi')); ?>",{
            method:'DELETE',
            headers:{'Content-Type':'application/json','X-CSRF-TOKEN':'<?php echo e(csrf_token()); ?>'},
...
<?php echo $__env->make('layouts.client.client', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\Workspace\laragon\www\qldatsan\resources\views/client/cart.blade.php ENDPATH**/ ?>